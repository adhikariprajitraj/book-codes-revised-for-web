"""
Standard Value Iteration Algorithm for Discounted MDPs
Equivalent to vi.c in the original C code
"""
...
def vi(tpm, trm, d_factor, epsilon):
    """
    Standard Value Iteration for Discounted MDPs.
...
def vi_with_verbose(tpm, trm, d_factor, epsilon, verbose=True):
    """
    ...
    try:
        result = vi(tpm, trm, d_factor, epsilon)
        return result
    finally:
        if not verbose:
